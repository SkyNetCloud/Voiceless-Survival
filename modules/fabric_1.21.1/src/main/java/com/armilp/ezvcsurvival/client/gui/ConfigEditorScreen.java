...
public class ConfigEditorScreen extends Screen {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
        ...
        context.drawCenteredTextWithShadow(textRenderer, this.title, centerX, titleY, 0xFFFFFFFF);

        // Subtítulo
        Text subtitle = Text.translatable("screen.ezvcsurvival.config_editor.subtitle");
        int subtitleY = titleY + 15;
        context.drawCenteredTextWithShadow(textRenderer, subtitle, centerX, subtitleY, 0xFFCCCCCC);
        ...
        int infoY = lineY + 12;
        context.drawCenteredTextWithShadow(textRenderer, info, centerX, infoY, 0xFFAAAAAA);

        // Versión en la esquina
        Text version = ModInfo.getVersion();
        int versionWidth = textRenderer.getWidth(version);
        context.drawText(textRenderer, version, this.width - versionWidth - 5, 5, 0xFFAAAAAA, true);
    }
